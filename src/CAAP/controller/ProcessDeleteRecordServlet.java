...
@WebServlet("/pages/deleterecord.html")
public class ProcessDeleteRecordServlet extends HttpServlet {
				...
				if(user.getStatus().equals("Admin") || user.getStatus().equals("Staff"))
				{
					if(user.getStatus().equals("Admin"))
					{
						...
						try {
							if (connection != null) 
							{
								SQLOperations.deleteRecord(id, connection);
								request.setAttribute("link", "tables.html");
								request.setAttribute("message", "Record "+id+" has been deleted successfully!");
