...
@WebServlet("/pages/settings.html")
public class GenerateSettingsPageServlet extends HttpServlet {
				...
				if(user.getStatus().equals("Admin") || user.getStatus().equals("Staff"))
				{
					try {
						if (connection != null) {
							...
							{
								ResultSet rs = SQLOperations.getAllUsers(connection);
								request.setAttribute("user", rs);
								getServletContext().getRequestDispatcher(
										"/pages/settingsAdmin.jsp").forward(request,
										response);
							}
							else
							{
								getServletContext().getRequestDispatcher(
										"/pages/settingsStaff.jsp").forward(request,
										response);
							}
