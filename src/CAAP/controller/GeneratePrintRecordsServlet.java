...
@WebServlet("/pages/print.html")
public class GeneratePrintRecordsServlet extends HttpServlet {
						...
						if (connection != null) {
							...
							if(request.getAttribute("type").equals("1"))
							{
								...
								getServletContext().getRequestDispatcher(
										"/pages/print.jsp").forward(request,
										response);
							}
							else
							{
								...
								try {
									date = formatter.parse(request.getParameter("from"));
								...
								try {
									date = formatter.parse(request.getParameter("to"));
									date2 = new Date(date.getTime());
								} catch (ParseException e1) {
									date=null;
								}
								ResultSet rs = SQLOperations.getAllRecordsRange(request.getParameter("mark"), date1, date2, connection);
								request.setAttribute("record", rs);
								
								getServletContext().getRequestDispatcher(
										"/pages/print1.jsp").forward(request,
										response);
							}
